...
export function getSelectionAndOptions_forAngleInstances(options) {
  ...
  if (artboards.length == 1) {
    return {
      ...
      artboardSelections: array.map((a, i, as) => {
        indexOfSelectedItem: () => 0;
      }),
      densitySelections: array.map((a, i, as) => {
        indexOfSelectedItem: () => 0;
      }),
      compressionSelections: array.map((a, i, as) => {
        indexOfSelectedItem: () => 0;
      }),
    };
  }
  ...
  rectangle = NSMakeRect(
    fisrtColumnWidth + secondColumnWidth,
    0,
    thirdColumnWidth,
    labelHeight
  );
  ...
  if (array.length > 1) {
    let targetLabels = array.map(function (a, i, as) {
      let rectangle = NSMakeRect(
        0,
        labelHeight + 4 + spacing * i,
        fisrtColumnWidth,
        labelHeight
      );
  ...
  let artboardImages = allArtboards.map((a) =>
    Shared.smallImagesFromArtboard(a)
  );

  let artboardSelections = array.map((a, index, as) =>
    Shared.popUpButtonsforRectangleIndexer_withTitleIndexer_andImageIndexer_defaultSelected_onIndex(
      (i) =>
        NSMakeRect(0, labelHeight + 4 + spacing * i + 16, fisrtColumnWidth, 28),
      artboardNames,
      artboardImages,
      index
    )
  ...
  let pixelDensitySelections = array.map((a, index, as) =>
    Shared.popUpButtonsforRectangleIndexer_withTitleIndexer_andImageIndexer_defaultSelected_onIndex(
      (i) =>
        NSMakeRect(
          fisrtColumnWidth,
          labelHeight + 4 + spacing * i + 16,
          secondColumnWidth,
          28
        ),
      pixelDensityNames,
  ...
  let compressionRatioSelections = array.map((a, index, as) =>
    Shared.popUpButtonsforRectangleIndexer_withTitleIndexer_andImageIndexer_defaultSelected_onIndex(
      (i) =>
        NSMakeRect(
          fisrtColumnWidth + secondColumnWidth,
...
function applyAngles(options) {
  let {
    angles,
    artboardsOnSelectPage: artboards,
    context,
    artboardsOnOtherPages: otherArtboards,
  } = options;
  ...
  if (artboards.length === 1) {
    ...
    angles.forEach(function (a, i, as) {
      let artboardSelectionIndex = response.artboardSelections[
        i
      ].indexOfSelectedItem();
      ...
      a.compressionRatio = response.compressionSelections[
        i
      ].indexOfSelectedItem();
    });
  }
...
export default function (context) {
  ...
  let artboardsOnOtherPages = Array.fromNSArray(document.pages())
